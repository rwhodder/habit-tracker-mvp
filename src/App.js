...
function App() {
  ...
  useEffect(() => {
    async function fetchHabits() {
      try {
        ...
        setError("Failed to load habits, defaulting to local state.");
        setHabitStates([
  ...
  async function handleNameSubmit(idx) {
    if (habitInputs[idx]?.trim()) {
      try {
        ...
        setError("Failed to save habit name. Please try again.");
      }
    }
  }
  ...
  async function handleLog(idx) {
    ...
    try {
      ...
      setError("Failed to log habit completion. Please try again.");
    }
  }
